...
class Resource {
    ...
    batch(requests) {
        ...
        //debugger;
        var ajax = axios(ajaxConfig);
        return ajax;
    }
    upload(modelName, isFilter, item, params) {
        ...
        axios(ajaxConfig).then(function(ret) {
    ...
    Factory(modelName) {
        ...
        Object.keys(actions).forEach(function(name) {
            ...
            resourceFactory[name] = function(a1, a2) {
                ...
                function next() {
                    ...
                    if (name === "logout" && !httpConfig["headers"]["authorization"]) {
                        return Promise.resolve({ status: 0, msg: "未设置authorization参数,无法注销!" });
                    }
                    if (hasBody) {
                        httpConfig.data = {
 ...
 */
class Route {
    constructor(template, baseurl) {
        this.template = template;
        this.baseurl = baseurl;
    }

    setUrlParams(config, params) {
